...
import { Calendar, Building2, ExternalLink, Briefcase } from "lucide-react";
...
export default function VacanciesPage() {
  ...
  }>(`/api/notices?category=vacancy&page=${currentPage}&limit=12`, currentPage);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div ref={headerRef}>
        <PageHeader title={t("notices.vacancy")} subtitle={t("notices.subtitle")}>
          <div className="flex items-center gap-2 text-sm text-muted-foreground">
            <Briefcase className="h-4 w-4" />
            <span>{pagination.total} vacancies</span>
          </div>
        </PageHeader>
      </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {notices.map((notice) => (
              <Card key={notice._id} className="hover:shadow-md transition-shadow">
                <CardHeader>
                  <Badge variant="default" className="shrink-0 w-fit">Vacancy</Badge>
